...
# Question 4.4: What is distributions of red wine based on each variables?
# Solution: Use Density Plot to show each attributes 
...
ui<- fluidPage(
  ...
  pageWithSidebar(
    ...
    sidebarPanel(
      ...
      textInput('caption', 'Plot Caption', value='Density Plot for each variables')
    ),
...
server <- function(input, output) {
  ...
  output$densityPlot <- renderPlot({
    ...
    p <- p + geom_density(alpha = 0.5) + ggtitle(input$caption)
