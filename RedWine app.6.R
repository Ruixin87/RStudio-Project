...
#Question 4.7: Based on the different quality grade what is corresponds different variables
#Solution: Use bar chart to present different variables corresponds each quality grade
...
ui <- fluidPage(
  ...
  pageWithSidebar(
    ...
    sidebarPanel(
      ...
      sliderInput('min_quality', 'Select Min Quality Grade on x Axis', min=3, max=8, value=3, step=1, round=0),
      sliderInput('max_quality', 'Select Max Quality Grade on x Axis', min=3, max=8, value=8, step=1, round=0),
      ...
      #selectInput('color', 'Measure Color', c('None', names(wine)), selected = names(wine)[[5]]),
      textInput('caption', 'Plot Caption', value='Bar Chart')
    ),
...
server <- function(input, output) {
  dataset <- reactive({
    #Directly subset and sample the wine data set
    wine[wine$quality >= input$min_quality & wine$quality <= input$max_quality, ][sample(nrow(wine[wine$quality >= input$min_quality & wine$quality <= input$max_quality, ]), 
                                                                                         
                                                                                         min(input$sampleSize, nrow(wine[wine$quality >= input$min_quality & wine$quality <= input$max_quality, ]))),]
  })
  ...
  output$barplot <- renderPlot({
    ...
    p <- ggplot(dataset(), aes_string(x = input$x, y = input$y, fill = input$x)) + geom_bar(stat = "identity")
    
    
    p <- p + ggtitle(input$caption)+theme_minimal()
